      ...
      ventasData.map(async (venta) => {
        ...
        const productosTransformados = await Promise.all(
          venta.productos.map(async (producto) => {
            const detallesProducto = await getDetallesProducto(producto.productoId); // Obtener detalles del producto
            return {
              ...producto,
        ...
        return {
          ...
          productos: productosTransformados, // Productos con detalles
          total: venta.total, // Monto total
          estado: venta.estadoPago, // Estado de pago
          fecha: new Date(venta.fecha).toLocaleDateString(), // Fecha formateada
          montoPendiente: venta.montoPendiente || 0, // Monto pendiente
        };
      })
    );
