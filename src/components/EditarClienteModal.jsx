...
const EditarClienteModal = ({ isOpen, onClose, cliente }) => {
  ...
  // Cargar los datos del cliente cuando el modal se abre
  useEffect(() => {
    if (cliente) {
      setFormData({
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Llamar a la función del contexto para editar el cliente
      await editCliente(cliente.id, formData);
