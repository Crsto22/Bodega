...
import { X, Save, Package, Tag, Calendar, BarChart4, ChevronDown, ChevronUp, Trash2 } from 'lucide-react';
import { useProduct } from '../context/ProductContext';

const EditarProductoDrawer = ({ isOpen, onClose, productToEdit }) => {
  ...
  useEffect(() => {
    if (productToEdit) {
      setFormData({
        ...
        stock: productToEdit.stock === 'No Especificado' ? '' : productToEdit.stock.toString() || '',
        fecha_vencimiento: productToEdit.fecha_vencimiento || ''
      });

      setTieneMarca(!!productToEdit.marca);
      setTieneFechaVencimiento(!!productToEdit.fecha_vencimiento);
    }
  }, [productToEdit]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const productoActualizado = {
        ...
        marca: tieneMarca ? formData.marca : '',
        fecha_vencimiento: tieneFechaVencimiento ? formData.fecha_vencimiento : ''
      };

      await editProduct(productToEdit.id, productoActualizado);
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-end transition-all">
      ...
      >
        <div className="flex justify-between items-center mb-4">
          <h3 className="font-bold text-xl text-gray-800">Editar Producto</h3>
          <button
            onClick={onClose}
        ...
        <div className="grid grid-cols-2 gap-3 mt-5">
          <button
            onClick={onClose}
            className="py-3 bg-gray-300 text-gray-700 rounded-lg font-medium flex items-center justify-center"
          >
            Cancelar
          </button>
          <button
            ...
            disabled={loading}
            className="py-3 bg-[#44943b] text-white rounded-lg flex items-center justify-center font-bold"
          >
            {loading ? (
...
export default EditarProductoDrawer;